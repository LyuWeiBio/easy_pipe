...
The [operator-only real-host runbook](real-host-acceptance.md) creates a
separate two-file, checksum-sealed supplemental package. It never replaces the
sealed candidate placeholder and remains `OPERATOR_GENERATED_UNREVIEWED`,
`PENDING_INDEPENDENT_REVIEW`, and `BLOCKED` until the human review steps occur.

The committed platform locks under `environments/locks/` are supply-chain
references generated by cross-platform metadata solves. They do not prove that
a candidate environment was created or executed on a native host, and their
`native_runtime_validation` state remains `pending`. A candidate-specific
`environment-explicit.txt` must describe the exact environment actually used
for that candidate and may bind to the matching lock and inventory hashes; a
committed lock must not simply be copied into the evidence bundle and presented
as runtime evidence.
...
The release-acceptance CI PR added native locked-environment execution,
sanitized CI evidence upload, and macOS controller-only compatibility. The
real-host tooling PR adds the bounded operator runner, sanitized supplemental
collector, and offline verification while preserving the incomplete operator
and reviewer states.
